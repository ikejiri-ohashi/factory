...
これらの対策により、画面遷移の回数が削減され、ユーザーストレスの軽減が実現しました。

### 一度に作られるSQL文を非同期通信を用いて削減する
...
これらの対策により、一度に発行するSQL文が削減され、アプリの動作遅延によるユーザーストレスを防止しました。
...
さらに各投稿の投稿主の情報表示も必要になるため非常に多くのN＋1問題の起因が潜んでおり、これらを回避するための対策に苦労しました。
